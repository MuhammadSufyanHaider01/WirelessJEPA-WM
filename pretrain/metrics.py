...
# IMAGENET_NORMALIZE = {"mean": [0.456, 0.406], "std": [0.224, 0.225]}
...
def log_example_inputs(views, log_label="train", num_examples=8):
    ...
    imgs = ((imgs - imgs_min) / (imgs_max - imgs_min) * 255).astype(np.uint8)
    # add a white channel to the last dimension to separate the views
    white_chan = 255 * np.ones((imgs.shape[0], imgs.shape[1], imgs.shape[2], 1), dtype=np.uint8)
    imgs = np.concatenate([imgs, white_chan], axis=-1)
...
def eval_feature_descriptors(backbone: nn.Module, dataset, batch_size: int = 64, input_size: int = 224, cfg_name=None, current_epoch=None):
    ...
    #         apply_prob=0.9,
    #     )
    minimal_transforms = ExclusiveComposeTransforms([])
