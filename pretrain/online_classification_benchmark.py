...
class OnlineLinearClassificationBenckmark:
    def __init__(
        ...
        self.val_transform = ExclusiveComposeTransforms([])

    def run_benchmarks_custom(self,
                       device,
                       dist_all_gather_fcn,
                       train_dataloader,
                       val_dataloader,
        ...
        val_dataloader.dataset.transform = self.val_transform
        ...
        if self.dist_rank == 0:
            ...
            #optimizer = optim.Adam(classifier.parameters(), lr=1e-3, weight_decay=0)  #Original scheuler:
            # ---- optimizer + scheduler ----
    ...
    def run_benchmarks(self,
        ...
        if self.dist_rank == 0:
            ...
            for p in classifier.parameters():
                p.requires_grad = True
            optimizer = optim.Adam(classifier.parameters(), lr=1e-3, weight_decay=0)
    ...
    def compute_features(self, dataloader, device):
        ...
        features = []
        targets = []
        for batch in tqdm(dataloader, desc=f"Computing features on {device}", miniters=100):
    ...
    def evaluate_knn_classifier(self, feature_bank, label_bank, val_features, val_labels, k=200, t=0.1, eval_batch_size=64):
        ...
        valid_topk = [min(k, self.num_classes) for k in self.topk]
        print(f"Evaluating kNN with k={k}, t={t}, valid_topk={valid_topk}")

        topk = mean_topk_accuracy(
            predicted_classes=predicted_classes, targets=val_labels_tensor, k=valid_topk
        )
