...
2. Train JEPA (`multi-block` by default) on the raw windows. Alternative representation baselines are intentionally deferred. The full-data GPU launcher uses a deterministic 90/10 pilot split. The corrected v4 path uses normalized (or centered) per-token latent regression with target/prediction scale and cosine diagnostics, the sparse context encoder, a learned mask token, and a three-block depthwise spatial predictor. Random/antenna/time ablations use a conservative `.25` base mask ratio, `3e-5` peak learning rate with a three-epoch warm-up and cosine decay, slow fixed EMA (`.9999`), clipped gradients, repeated validation-mask passes, and AdamW groups that exclude bias/norm parameters from decay. GroupNorm keeps predictor behavior identical in train/eval; context BatchNorm remains live by default, with `--freeze-bn` available as an explicit ablation. An optional `--mask-curriculum-epochs` ramps random/time/multi-block ratios from `--mask-ratio-start` to the target; antenna masks stay discrete and fixed. The launcher writes versioned `*_gpu_v4.pt` checkpoints; ratio sweeps remain explicit ablations:
...
JEPA CSV/TensorBoard logs include averaged train/validation loss, validation-repeat count, effective learning rate, EMA momentum, gradient norm, target/prediction feature standard deviations and norms, cosine alignment, representation-validity status, and mask configuration. A checkpoint is not accepted as improved when held-out target or prediction variance falls below `--min-feature-std`. Antenna/time ablations should first be compared at the same conservative fixed ratio before running the registered `.25/.5/.75` ratio sweep. Existing v2/v3 checkpoints remain loadable through their legacy predictor/raw-loss metadata, but new comparisons should use v4 checkpoints only. The controller logs entropy coefficient, normalized value-target scale, deterministic evaluation return/KPIs, `approx_kl`, `clip_fraction`, action log-standard-deviations, and rollout count. A stochastic episode return is not used as the best-checkpoint criterion; `_best.pt` is selected by deterministic held-out return.
