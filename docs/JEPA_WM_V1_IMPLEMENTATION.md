...
2. Train JEPA (`multi-block` by default) on the raw windows. Alternative representation baselines are intentionally deferred. The full-data GPU launcher uses a deterministic 90/10 pilot split. Sparse random/antenna/time ablations use a conservative `.25` base mask ratio, `1e-4` learning rate with a ten-epoch warm-up and cosine decay, slow EMA (`.999` to `.9999`), clipped gradients, repeated validation-mask passes, and frozen BatchNorm statistics/affine parameters. The launcher writes versioned `*_gpu_v3.pt` checkpoints; ratio sweeps remain explicit ablations:
